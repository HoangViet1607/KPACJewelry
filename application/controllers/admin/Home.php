...
class Home extends MY_Controller {
    ...
    public function index() {
       ...
       if(sizeof($pro_total) != 0){
        ...
        //Đơn hàng
        $str12 = "SELECT COUNT(*) as sl FROM transaction WHERE transaction.`status`= 3 LIMIT 0,10000";
        ...
        if ($this->input->post()) {
            if ($this->input->post('year') != 0) {
                ...
                $str1 = "Select MONTH(date) as 'Thang',SUM(amount) as money
                        FROM transaction
                        
                        WHERE status = 3 AND YEAR(date) = " . $year_post . " 
                ...
                $str2 ="Select YEAR(date) as 'Thang',SUM(amount) as money
                        FROM transaction
        ...
        if ($this->input->post('start_date') && $this->input->post('end_date')) {
            ...
            $this->data['selected_start_date'] = $start_date;
            $this->data['selected_end_date'] = $this->input->post('end_date');
            // Truy vấn tổng doanh thu trong khoảng thời gian
            $str2 = "SELECT SUM(amount) as money
                     FROM transaction
                     WHERE status = 3 
                     AND date BETWEEN '" . $start_date . "' AND '" . $end_date . "'";
            $data_big2 = $this->transaction_model->query3($str2);
