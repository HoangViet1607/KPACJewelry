...
class Product extends MY_Controller {
    ...
    public function index() {
        ...
        $this->db->select('product.id as id, product.name as name, price, discount, image_link, view, buyed, catalog.name as namecatalog, GROUP_CONCAT(sizes.name SEPARATOR ", ") as sizename, GROUP_CONCAT(sizedetail.quantity SEPARATOR ", ") as quantities');
        $this->db->join('catalog', 'catalog.id = product.catalog_id');
        $this->db->join('sizedetail', 'sizedetail.product_id = product.id', 'left');
        $this->db->join('sizes', 'sizes.id = sizedetail.size_id', 'left');
    ...
    public function del()
    {
        ...
        $sizedetails = $this->sizedetail_model->get_list($input);

        if (!empty($sizedetails)) {
            foreach ($sizedetails as $detail) {
                $this->sizedetail_model->delete($detail->id);
            }
        }

        if ($this->product_model->delete($id)) {
            $this->session->set_flashdata('message_success', 'Xóa sản phẩm thành công');
        } else {
            $this->session->set_flashdata('message_fail', 'Xóa sản phẩm thất bại');
        }
