                    ...
                    <tbody>
                        <?php foreach ($product as $value) { ?>
                            <tr>
                                ...
                                <?php
                                    if (empty($value->sizename) || empty($value->quantities)) {
                                        echo '<strong>Hết hàng</strong>';
                                    } else {
                                       // Combine sizes and quantities
                                        $sizes = explode(',', $value->sizename);
                                        $quantities = explode(',', $value->quantities);
                                        $size_quantity = '';
                                    
                                        foreach ($sizes as $index => $size) {
                                            $quantity = isset($quantities[$index]) ? $quantities[$index] : '0';
                                            $size_quantity .= $size . ' | Số lượng: ' . $quantity . '<br>';
                                        }
                                        echo '<strong>' . $size_quantity . '</strong>';
                                    }
                                    ?>
                                </td>
                                ...
                                <td class="list_td aligncenter">
                                    ...
                                    <a href="<?php echo admin_url('product/del/' . $value->id); ?>" title="Xóa"> <span class="glyphicon glyphicon-remove" onclick=" return confirm('Bạn chắc chắn muốn xóa')"></span> </a>
                                </td>    
                            </tr>
                        <?php } ?>
