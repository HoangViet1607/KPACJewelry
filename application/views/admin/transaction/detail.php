			...
			<div class="panel-body">
				...
				<div class="table-responsive">
					...
					<?php if ($transaction->status == 0) { ?>
    					<a href="<?php echo admin_url('transaction/update_status/1/'.$transaction->id); ?>" class="btn btn-success">Xác nhận đơn hàng</a>
    					<a href="<?php echo admin_url('transaction/update_status/4/'.$transaction->id); ?>" class="btn btn-danger">Hủy đơn hàng</a>
					<?php } elseif ($transaction->status == 1) { ?>
    					<a href="<?php echo admin_url('transaction/update_status/2/'.$transaction->id); ?>" class="btn btn-success">Đang giao hàng</a>
					<?php } elseif ($transaction->status == 2) { ?>
    					<a href="<?php echo admin_url('transaction/update_status/3/'.$transaction->id); ?>" class="btn btn-success">Giao hàng thành công</a>
    					<a href="<?php echo admin_url('transaction/update_status/5/'.$transaction->id); ?>" class="btn btn-warning">Trả Hàng</a>
					<?php } ?>
